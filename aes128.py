...
hex_symbols_to_int = {'a': 10, 'b': 11, 'c': 12, 'd': 13, 'e': 14, 'f': 15}                             #for hex alues above 9
...
#rcon is fixed for every algorithm
...
def key_expansion(key):
    ...
    about algorithm is is in AES standard

    """

    key_symbols = [ord(symbol) for symbol in key]                           #gets ascii of the keys in key_symbols

    # ChipherKey shoul contain 16 symbols to fill 4*4 table. If it's less
    # complement the key with "0x01"
    if len(key_symbols) < 4 * nk:                                           #check len is 16
        for i in range(4 * nk - len(key_symbols)):
            key_symbols.append(0x01)                                    #if len < 16 add 1 at vacant spaces
    ...
    for col in range(nk, nb * (nr + 1)):  # col - column number             nr=10
        if col % nk == 0:
            # take shifted (col - 1)th column...
            # fill vacant space in key_schedule by 16
            tmp = [key_schedule[row][col - 1] for row in range(1, 4)]           #add all except first row to temp
            tmp.append(key_schedule[0][col - 1])                                        #add first row at last
